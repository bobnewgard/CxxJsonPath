...
bool enable_test_18 = true;
bool enable_test_19 = true;
...
bool test_a_path(const string & arg_m, JsonFind & arg_c, string & arg_p, Token & arg_et, string & arg_es)
{
    ...
    if (ret)
    {
        ...
        else
        {
            if (debug_msg)
            {
                msg.cerr_inf("pass, find() returned string with expected element_str" + SP + DQ + obsv.element_str + DQ);
            }
            else
            {
                msg.cerr_inf("pass, find() returned expected element");
...
int main(int argc, char * argv[])
{
    ...
    {
        bool                 ret    = true;
        string               cstr   = "{\"frame_len\":\"\"}";
        unique_ptr<JsonFind> json18 = unique_ptr<JsonFind>(new JsonFind());

        try
        {
            json18->set_search_context(cstr);
        }
        catch (JsonFindErr & err)
        {
            msg.cerr_err("test18: catch() while parsing empty string value");
            msg.cerr_err(err.get_msg());
            ret = false;
        }

        msg.cerr_inf("test18: successfully parsed empty string value");
    ...
    if (enable_test_19)
    {
        bool                 ret    = true;
        string               cstr   = "{\"\":\"frame_len\"}";
        unique_ptr<JsonFind> json19 = unique_ptr<JsonFind>(new JsonFind());

        try
        {
            json19->set_search_context(cstr);
        }
        catch (JsonFindErr & err)
        {
            msg.cerr_err("test18: catch() while parsing empty key string");
            msg.cerr_err(err.get_msg());
            ret = false;
        }

        msg.cerr_inf("test18: successfully parsed empty key string");
